...
let testFuncs =
    ...
    |> Seq.mapi (fun index instr ->
        let func =
            fun value () -> Expect.equal instr value "Healthcheck must be disabled"
...
let tests =
    ...
    |> List.ofSeq
    |> List.append [
        testCase "healthcheck w/o options test"
        <| fun () ->
            let expected =
                HealthcheckInstruction.Create ("curl -f http://localhost/ || exit 1")

            let actual = hc "curl -f http://localhost/ || exit 1"

            Expect.equal actual expected "Healthcheck must be w/o options"

        testCase "healthcheck with options test"
        <| fun () ->
            let expected =
                Healthcheck
                    {
