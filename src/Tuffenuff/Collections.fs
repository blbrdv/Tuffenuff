...
type Collection<'T>(elements : IEnumerable<'T>) =
    ...
    member __.Collection with get() = elements
    ...
    member __.Append(coll2 : Collection<'T>) =
        Seq.append elements coll2
        |> Collection
...
type Dict<'TKey, 'TValue when 'TValue: equality and 'TKey: comparison>(elements : Map<'TKey, 'TValue>) =

    inherit ReadOnlyDictionary<'TKey, 'TValue>(elements)


    new(elements : seq<'TKey * 'TValue>) =
        Dict( Map elements )
    ...
    member __.Append(map2 : Map<'TKey, 'TValue>) =
        Map.fold (fun acc key value -> Map.add key value acc) elements map2
        |> Dict
